...
However, if you do have a grouping variable (participant, condition, type of something), it will generally work better with a lot of R applications to make sure R knows it is a factor. To change a character column into a factor use `as.factor()` on the column name.

To make a categorical column be read as a factor, you must overwrite the saved column by saving an identical copy of the column converted to a factor over it.
